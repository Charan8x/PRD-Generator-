...
const MainPage = ({ token, onLogout }) => {
  ...
  const handleSaveAndRegenerate = async (e) => {
    ...
    if (!editDesc.trim()) { setEditValError('Project description is required.'); return; }
        ...
        {currentScreen === 'input' ? (
          <>
            ...
            <section className="form-section">
              <ProjectForm token={token} onGenerationSuccess={handleGenerationSuccess} />
            </section>
          </>
        ) : (
          <section className="results-section" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            {isEditing ? (
                ...
                <form onSubmit={handleSaveAndRegenerate} className="project-form">
                  ...
                  <div className="form-group">
                    ...
                    <textarea
                      ...
                      value={editDesc}
                      disabled={editLoading}
                      onChange={(e) => { setEditDesc(e.target.value); if (e.target.value.trim()) setEditValError(''); }}
                      rows={5}
                    />
                    {editValError && (
                      <p style={{ color: 'var(--error)', marginTop: '4px', fontSize: '13px', fontWeight: 500 }}>
                        ❌ Project description is required.
                      </p>
                    )}
                  </div>
                  <div style={{ display: 'flex', gap: '12px' }}>
                    <button type="submit" className="btn-primary" style={{ width: 'auto' }} disabled={!editName.trim() || editLoading}>
                      {editLoading ? 'Regenerating...' : 'Save & Regenerate'}
                    </button>
                    <button type="button" className="btn-secondary" style={{ width: 'auto' }} disabled={editLoading} onClick={handleCancelEdit}>
                      Cancel
                    </button>
                  </div>
                </form>
              ...
              <div className="content-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
